...
export const DistributionManager: React.FC<{ lang: Language, onNavigate?: (tab: any) => void }> = ({ lang, onNavigate }) => {
  ...
  const handlePrepare = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!trackTitle || !selectedFile) return;
    setLoading(true);
    try {
      const res = await prepareDistributionConfig(trackTitle, lang);
      setFormats(res);
      // Initialize status
          ...
          {!formats.length ? (
            <form onSubmit={handlePrepare} className="space-y-6 animate-in fade-in">
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Nome do Lançamento</label>
                <input 
                  type="text" 
                  placeholder="Ex: Vida de Neon (Deluxe)"
              ...
              <button 
                type="submit"
                disabled={loading || !trackTitle || !selectedFile}
...
const PlatformCard: React.FC<{ format: DistributionFormat, status: 'pending' | 'syncing' | 'uploading' | 'metadata' | 'done' }> = ({ format, status }) => {
  ...
  const getUrl = () => {
    switch (format.platform) {
      case 'YouTube': return 'https://www.youtube.com/@soundofstreets-v1p';
      case 'Instagram Reels': return 'https://www.instagram.com/soundofstreets/';
      case 'TikTok': return 'https://www.tiktok.com/@soundofstreets-v1p';
      case 'Facebook Watch': return 'https://www.facebook.com/watch/';
      default: return 'https://www.youtube.com/@soundofstreets-v1p';
    }
  };
