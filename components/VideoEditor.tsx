...
const translations = {
  pt: {
    ...
    shortsDesc: "Identifique os momentos virais do seu vídeo para TikTok e Reels.",
    thumbTitle: "Editor de Thumbnails IA",
    thumbDesc: "Crie e personalize sua capa profissional para as redes sociais.",
    audioTitle: "Otimizador de Áudio PRO",
    audioDesc: "Visualize e otimize seu áudio para os padrões das plataformas.",
    btnAnalyze: "Analisar Melhores Momentos",
    btnGenerate: "Gerar Base com IA",
    btnDownload: "Baixar Thumbnail",
    btnClean: "Limpar Áudio IA",
    labelCreatorName: "Nome do Criador",
    labelContentTitle: "Título do Conteúdo",
    labelColor: "Cor do Brilho",
    placeholderScene: "Descreva a cena (ex: Escritório moderno, fundo tech, vibe vibrante)...",
  ...
  en: {
    ...
    shortsDesc: "Identify viral moments of your video for TikTok and Reels.",
    thumbTitle: "AI Thumbnail Editor",
    thumbDesc: "Create and customize your professional social media cover.",
    audioTitle: "Audio Optimizer PRO",
    audioDesc: "Visualize and optimize your audio for platform standards.",
    ...
    labelColor: "Glow Color",
    placeholderScene: "Describe the scene (ex: Modern office, tech background, vibrant vibe)...",
...
export const VideoEditor: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const handleDownloadClip = () => {
    ...
    setTimeout(() => {
      ...
      const dummyContent = "CONTENT_CLIP_DATA_" + Date.now();
      const blob = new Blob([dummyContent], { type: 'video/mp4' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `CONTENT-CLIP-${youtubeId || 'VIRAL'}.mp4`;
  ...
  const downloadCanvas = () => {
    ...
    if (!canvas) return;
    const link = document.createElement('a');
    link.download = `CONTENT-THUMB-${editorText.title || 'PROJECT'}.png`;
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                      <TypeIcon size={12} /> {t.labelCreatorName}
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                      <TypeIcon size={12} /> {t.labelContentTitle}
                    </label>
                    <input
                      type="text"
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white focus:border-yellow-500"
